...
Motor de atendimento automatizado multimodal com **Rust (Axum)**, **Google Gemini 1.5 Pro / Flash**, **SQLite FTS5** e integração oficial com a API da **uTalk (Umbler)**.
...
- **🧠 Google Gemini & Function Calling (Tool Calling)**: Permite ao robô consultar APIs externas em tempo real (estoques, ERPs, sistemas de frete) antes de responder ao cliente.
...
- **🎨 Painel de Controle de Alta Performance**: Interface moderna inspirada no estilo *UIDict / Glassmorphism*, permitindo habilitar/pausar o robô, alterar prompts e cadastrar APIs dinâmicas sem reiniciar a aplicação.
